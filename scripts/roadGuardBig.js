...
//添加扇形小区
function addSectorXQ(latlng,radius,start,stop,name,lacci,hotspot,hotId){
...
//拼接url参数
function constructParamsRoad(arr){
    var temStr = '&hotspot=';
...
function initBaiduMap(){
    ...
    }).setView([31.312888,121.441563], 15);
    //自定义地图数据源
	//tileSize==512
            ...
            if(lengthRows){
                ...
                for(var i=0; i<lengthRows; i++){
                    if(data.rows[i].latitude != undefined && data.rows[i].latitude != null && data.rows[i].latitude != "" && data.rows[i].longitude != undefined && data.rows[i].longitude != null && data.rows[i].longitude != ""){
                        ...
                        //为小区绑定popup
                        var heatPopup = L.popup({maxWidth:1000,maxHeight:1000,offset:L.point(0, 5),closeButton:true, closeOnClick:false})
                            .setLatLng(point);
                        heatPopup.setContent('<iframe width="680px" frameborder=no height="460px" src='+"pagesDSN/sitePieChartsBig.html?lacci="+lacci+"&hotspot="+encodeURIComponent(heatIndexDis) + "&name=" + encodeURIComponent(popupContent) +'></iframe>');
...
function addRectangleTool(){
    ...
    map.on('draw:edited', function (e) {
        ...
        //分组计算
        var groupLayers = groupSectorAfterMove(layersArr,0.0005);
        var updateArr = [];
...
function addXQ2Map(str,flag){
    ...
    heatMapLayer && (switchControl.removeLayer(heatMapLayer), map.removeLayer(heatMapLayer),cacheHeatMapData=[]);
            ...
            for(var i= 0, len = data.length; i<len;++i){
                if ((data[i].lat !="" && data[i].lon != "") && (data[i].lat != null && data[i].lon != null)) {
                    ...
                    if(isNumber(latHeatMap) && isNumber(lngHeatMap)){
                        ...
                        if(data[i].hori_direc_angle === null || data[i].mechanical_dip_angle === null) {continue;}
...
function beginAddXQ1(arr,hotspot){
    ...
    for(var i= 0, lenI=arr.length; i<lenI; ++i){
        ...
        var heatPopup = L.popup({maxWidth:800,maxHeight:800,offset:L.point(0, 5),closeButton:true, closeOnClick:false})
            .setLatLng([lat,lng]);
        heatPopup.setContent('<iframe width="680px" frameborder=no height="460px" src='+"pagesDSN/sitePieChartsBig.html?"+lacci+"!"+encodeURIComponent(hotspot) + "&" + encodeURIComponent(name) +'></iframe>');
...
function beginAddXQ(arr,hotspot){
    ...
    for(var i= 0, lenI=arr.length; i<lenI; ++i){
        ...
        var heatPopup = L.popup({maxWidth:800,maxHeight:800,offset:L.point(0, 5),closeButton:true, closeOnClick:false})
            .setLatLng([lat,lng]);
        heatPopup.setContent('<iframe width="680px" frameborder=no height="460px" src='+"pagesDSN/sitePieChartsBig.html?lacci="+lacci+"&hotspot="+encodeURIComponent(hotspot) + "&name=" + encodeURIComponent(name) +'></iframe>');
...
function showAllIndexInfo(disName){
    $('#userData').empty();
    var src4 = "pagesDSN/ywzlzbBigRight.html?name=";
    constructDomTrend(disName,'userData',src4);
    $('#dataThroughPut').empty();
    var src1 = "pagesDSN/rmzdBigRight.html?name=";
    constructDomTrend(disName,'dataThroughPut',src1);
    $('#hotApp').empty();
    var src5 = "pagesDSN/rmyyBigRightGrid.html?name=";
    constructDomTrend(disName,'hotApp',src5);
    $('#hotTerminal').empty();
    var src6 = "pagesDSN/wlzlzb1BigRight.html?name=";
...
function addImportantData(){
    ...
    var bigGroupData = ['高架','地铁','高速','高铁','交通枢纽','景点','商业区'];
    var temStr = constructParamsRoad(bigGroupData);
    ...
    $.ajax({
        ...
        .done(function(data){
            ...
            for(var i= 0,lenI=bigGroupData.length; i<lenI; ++i){
                var nameVal = bigGroupData[i];
                for(var name in data){
                    if(name === nameVal){
                        var targetObj = data[name];
                        var temVal = (targetObj.total/10000).toFixed(2);
                        jqArr[i].innerHTML = temVal + "<span>&nbsp;万</span>";
                    }
                }
            }
            var time = targetObj.time.substring(11,16);
            $('.ct_time').html(time);
        });
    //查流量
    $.ajax({
        ...
        .done(function(data){
            ...
            for(var i= 0,lenI=bigGroupData.length; i<lenI; ++i){
                ...
                for(var name in data){
                    if(name === nameVal){
                        var targetObj = data[name];
                        var temVal = (targetObj[indexName]/1024/1024).toFixed(2);
                        jqArr[i].innerHTML = temVal + "<span>&nbsp;GB</span>";
                    }
                }
            }
        });
...
function callHWL(){
    var bigGroupData = ['高架','地铁','高速','高铁','交通枢纽','景点','商业区'];
    callServerHWL(bigGroupData,0);
}
function callServerHWL(arr,j){
    var hotUrl = 'http://' + baseUrl + ':8080/services/ws/area/pm';
    var hotSpot = arr[j];
    var paras = JSON.stringify({"hotspot":hotSpot,"domains":"2ghwl,3ghwl","time":""});
    var jqArr = $('.cL1_c2 h1');
    $.ajax({
        type : "post",
        async : true, //异步执行
        ...
        .done(function(data){
            ...
            var hwlData2G = 0,
                hwlData3G = 0;
            var totalHWL = 0;
            data['2ghwl'] && (hwlData2G = data['2ghwl']);
            data['3ghwl'] && (hwlData3G = data['3ghwl']);
            totalHWL = hwlData2G + hwlData3G;
            totalHWL = parseFloat(totalHWL).toFixed(2);
            jqArr[j++].innerHTML = totalHWL + "<span>&nbsp;Erl</span>";
            //console.log(parseFloat(totalHWL).toFixed(2));
            j <7 && callServerHWL(arr,j);
        });
}
//重点区域下二级子区域
function addDetailedInfo(str){
    var time = getCurrentTimeMin(15);
    ...
    $.ajax({
        ...
        .done(function(data){
            for(var i= 0,lenI=bigGroupData.length; i<lenI; ++i){
                ...
                //console.log(bigGroupData[i],temVal);
                $('#secondMenuCustom td.ctd2').eq(i).html( temVal + '<span>'+ '&nbsp' + '万' + '</span>' );
            }
        });
}
function calculateDetailedParas(){
    var detailedData = $('#secondMenuCustom td.ctd1');
...
//重点区域下三级子区域
function addDetailedInfoThird(str){
    var time = getCurrentTimeMin(15);
    var bigGroupData = calculateDetailedParasThird();
    var temStr = constructParamsRoad(bigGroupData);
    var urlTotalCount = 'http://' + baseUrl + ':8080/stream/totalnum/hotspots?time=' + time + temStr;
    //console.log(bigGroupData);
    //查用户数
    $.ajax({
        url: urlTotalCount,
        type: 'get',
        ...
        .done(function(data){
            for(var i= 0,lenI=bigGroupData.length; i<lenI; ++i){
                var temVal = (data[bigGroupData[i]].total/10000).toFixed(2);
                //console.log(bigGroupData[i],temVal);
                $('#thirdMenuCustom td.ttd2').eq(i).html( temVal + '<span>'+ '&nbsp' + '万' + '</span>' );
            }
        });
}
function calculateDetailedParasThird(){
    var detailedData = $('#thirdMenuCustom td.ttd1');
    //console.log(detailedData);
    var parasArr = [];
    for(var i= 0,lenI=detailedData.length; i<lenI; ++i){
        var temStr = detailedData[i].innerHTML;
...
//二级菜单
function createCustomDiv(str){
    //console.log(str);
    var jqArr = $('.elevated_txt');
    for(var i= 0,lenI=jqArr.length; i<lenI; ++i){
        var domName = jqArr[i].innerHTML;
        if(domName === str){
            var parentDiv = $(jqArr[i]).parent();
            //console.log(parentDiv.width(),parentDiv.height(),parentDiv.position());
            var topVal = parentDiv.position().top,
                leftVal = parentDiv.position().left + parentDiv.width();
            createMenuDiv(topVal,leftVal,str);
        }
    }
    function createMenuDiv(top,left,str){
        var customDiv = document.getElementById("secondMenuCustom");
        customDiv != null && (customDiv.parentNode.removeChild(customDiv));
        var parentdiv=$('<div></div>');        //创建一个父div
        parentdiv.attr('id','secondMenuCustom');        //给父div设置id
        parentdiv.addClass('img-preview');
        parentdiv.css({
            'position':'absolute', 'left': left + 100, 'top': top + 50, 'width': '300px', 'border': '1px solid #003250', 'background-color': '#003250', 'color': 'white'
        });
        parentdiv.appendTo('body');            //将父div添加到body中
        $.ajax({
            url: 'http://' + baseUrl + ':8080/services/ws/fast_query/area/re/re_areaHotRel?hotspot=' + encodeURIComponent(str) + '&hot_type=0,1',
            type: 'get',
            dataType: 'json'
        })
            .done(function(data){
                ...
                var useData = [];
                for(var i= 0,lenI=data.length; i<lenI; ++i){
                    var hot_type = data[i].hot_type;
                    var hot_name =data[i].hot_name;
                    var area_name = data[i].area_name;
                    if(hot_name === str) continue;
                    if(hot_type === 1) continue;
                    useData.push(hot_name);
                }
                var trLen = useData.length;
                createTable(trLen,5,useData);
            });
    }
    function createTable(rowCount,cellCount,data){
        var table=$("<table border=\"0\" style='width: 100%; text-align: center;'>");
        table.appendTo($("#secondMenuCustom"));
        if(rowCount === 0) {
            var tr=$("<tr></tr>");
            tr.appendTo(table);
            var td=$("<td class='ctd1'>"+ '暂无' + '<span>'+ '子菜单数据' + '</span>' + "</td>");
            td.appendTo(tr);
            tr.appendTo(table);
            $("#secondMenuCustom").append("</table>");
            return;
        }
        for(var i=0;i<rowCount;i++)
        {
            ...
            tr.appendTo(table);
            var name = data[i];
            var preName = name.substring(0,2);
            var lastName = name.substring(2);
            var td=$("<td class='ctd1'>"+ preName + '<span>'+ lastName + '</span>' + "</td>");
            td.appendTo(tr);
            var td=$("<td class='ctd2'>"+ '6.33' + '<span>' + '&nbsp;' + '万' + '</span>' + "</td>");
            td.appendTo(tr);
        ...
        $("#secondMenuCustom").append("</table>");
        addDetailedInfo();
        $('.ctd1').on('click',function(){
            //console.log(this.innerText);
            heatIndexDis = this.innerText;
            heatFlag = false;
            addXQ2Map(heatIndexDis,heatFlag);
            showAllIndexInfo(heatIndexDis);
            createThirdCustomDiv(heatIndexDis);
        });
    }
}
//三级菜单
function createThirdCustomDiv(str){
    var parentDiv = $('#secondMenuCustom');
    ...
    createMenuDiv(topVal,leftVal,str);
    function createMenuDiv(top,left,str){
        var customDiv = document.getElementById("thirdMenuCustom");
        customDiv != null && (customDiv.parentNode.removeChild(customDiv));
        var parentdiv=$('<div></div>');        //创建一个父div
        parentdiv.attr('id','thirdMenuCustom');        //给父div设置id
        parentdiv.addClass('img-preview');
        parentdiv.css({
            'position':'absolute', 'left': left + 20, 'top': top , 'maxWidth': '600px', 'maxHeight': '500px', 'overflow': 'auto','border': '1px solid #003250', 'background-color': '#003250', 'color': 'white'
        });
        parentdiv.appendTo('body');            //将父div添加到body中
        $.ajax({
            url: 'http://' + baseUrl + ':8080/services/ws/fast_query/area/re/re_areaHotRel?hotspot=' + encodeURIComponent(str) + '&hot_type=1',
            type: 'get',
            dataType: 'json'
        })
            .done(function(data){
    ...
    function createTable(rowCount,cellCount,data){
        var table=$("<table border=\"0\" style='width: 100%; text-align: center;'>");
        table.appendTo($("#thirdMenuCustom"));
        if(rowCount === 0) {
            var tr=$("<tr></tr>");
            tr.appendTo(table);
            var td=$("<td class='ttd1'>"+ '暂无' + '<span>'+ '子菜单数据' + '</span>' + "</td>");
            td.appendTo(tr);
            tr.appendTo(table);
            $("#thirdMenuCustom").append("</table>");
            return;
        }
        for(var i=0;i<rowCount;i++)
        {
            ...
            tr.appendTo(table);
            var name = data[i];
            var preName = name.substring(0,3);
            var lastName = name.substring(3);
            var td=$("<td class='ttd1'>"+ preName + '<span>'+ lastName + '</span>' + "</td>");
            td.appendTo(tr);
            var td=$("<td class='ttd2'>"+ '6.33' + '<span>' + '&nbsp;' + '万' + '</span>' + "</td>");
            td.appendTo(tr);
            //for(var j=0;j<cellCount;j++){
            //    var td=$("<td>"+i*j+"</td>");
            //    td.appendTo(tr);
        ...
        $("#thirdMenuCustom").append("</table>");
        addDetailedInfoThird();
        $('.ttd1').on('click',function(){
            //console.log(this.innerText);
            heatIndexDis = this.innerText;
            heatFlag = false;
            showAllIndexInfo(heatIndexDis);
...
function showNavigateMenu(){
    document.getElementById('showNavigate').style.visibility = 'hidden';
    document.getElementById('hideNavigate').style.visibility = 'visible';
    document.getElementById('navigateMenu').style.visibility = 'visible';
}
//隐藏导航菜单
function hideNavigateMenu(){
    document.getElementById('showNavigate').style.visibility = 'visible';
    document.getElementById('hideNavigate').style.visibility = 'hidden';
    document.getElementById('navigateMenu').style.visibility = 'hidden';
    clearSecondMenu();
    clearThirdMenu();
}
//切换地铁和地图
function exchange(str){
    if(str === '地铁'){
        ...
        document.getElementById('infoLocation').style.visibility = 'hidden';
        document.getElementById('exchangeSub').style.visibility = 'visible';
        var height = $(document).height(),
            width = $(document).width();
        var subUrl = 'http://' +baseUrl + ':8080/INAS/pages/local-lsm/metro/TOSFlashWS.html';
        $('#exchangeSub').html('<iframe width=' + width + ' frameborder=no scrolling=no height=' + height + ' src=' + subUrl + '>' + '</iframe>');
        //$('#exchangeSub').html('<iframe width=3100px frameborder=no scrolling=no height=1100px src=' + subUrl + '>' + '</iframe>');
        document.getElementById('mapOrSub').value = '地图';
    }else if(str === '地图'){
        document.getElementById('map').style.visibility = 'visible';
        document.getElementById('infoLocation').style.visibility = 'visible';
        document.getElementById('exchangeSub').style.visibility = 'hidden';
        document.getElementById('mapOrSub').value = '地铁';
    }
}
//记忆转换法
