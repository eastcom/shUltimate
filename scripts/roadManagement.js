...
function initCls(){
    $("div[name='left_tab']").click(function(){
        ...
        if(tab_id=="tab6"){
            ...
            $("#tab6").find(".circle_line").eq(0).show();
            $("#tab2").animate({top:"-"+moveCnt+'px'});
            $("#tab3").animate({top:"-"+moveCnt+'px'});
            $("#tab4").animate({top:"-"+moveCnt+'px'});
            $("#tab5").animate({top:"-"+moveCnt+'px'});
...
function initBaiduMap(){
    ...
    }).setView([31.195769,121.338157], 17);
    //自定义地图数据源
    //tileSize==512
...
function addPolygonData2Map(){
    ...
    var sfz = [
        {lat: 31.366316, lng: 121.265882},
        {lat: 31.359134, lng: 121.277046},
        {lat: 31.367635, lng: 121.302366},
        {lat: 31.37577, lng: 121.277217}
    ];
    ...
    constructPolygon(sfz,'收费站');
    map.addLayer(polygonLayer);
    map.addLayer(popupLayer);
    polygonLayer.bringToFront();
    map.setView([31.195769,121.338157], 13);
}
//获取颜色值
function getColor(d) {
    return d < 0 ? '#FFFFFF' :
...
function constructLabel(latlng,dataVal){
    var heatPopup = L.popup({maxWidth:800,maxHeight:800,offset:L.point(0, 5),closeButton:true, closeOnClick:false})
        .setLatLng(latlng);
    heatPopup.setContent('<iframe width="410px" frameborder=no height="300px" src='+"pages/queryImportantInfo.html?name=" + encodeURIComponent(dataVal) + '></iframe>');
    ...
    ).bindPopup(heatPopup).addTo(labelLayer);
}
    ...
    $.ajax({
        ...
        .done(function(data){
            ...
            for(var name in data){
                ...
                name === '寺庙' && ( smData = (targetObj.total/10000).toFixed(2));
                name === '收费站' && ( sfzData = (targetObj.total/10000).toFixed(2));
            }
            ...
            $('#sim .custom-index-info-left-wx b').html(smData);
            $('#sfz .custom-index-info-left-wx b').html(sfzData);
    ...
    $.ajax({
        ...
        .done(function(data){
            ...
            for(var name in data){
                ...
                name === '寺庙' && ( smData = (targetObj[indexName]/1024/1024).toFixed(2));
                name === '收费站' && ( sfzData = (targetObj[indexName]/1024/1024).toFixed(2));
            }
            ...
            $('#sim .custom-index-info-right-wx b').html(smData);
            $('#sfz .custom-index-info-right-wx b').html(sfzData);
        });
    //*/
}
//重点保障区域详细区域数据
...
function addXQ2Map(str,flag){
    ...
    heatMapLayer && (switchControl.removeLayer(heatMapLayer), map.removeLayer(heatMapLayer), heatMapLayer = null);
...
function addMarkerClusters(url,hotspot,flag){
    ...
    $.ajax({
        ...
        .done(function(data){
            ...
            //console.log(heatMap)
            beginAddXQ(heatMap,hotspot,flag);
            addResultNow(heatMap,hotspot,flag);//当前热力图渲染
        });
}
function beginAddXQ(arr,hotspot,flag){
    //自定义图标
    var marker2G = L.icon({
    ...
    //flag === true && (map.setView(center,13));
    flag === true && (map.addLayer(markersLayer), map.setView(center,13));
    switchControl.addOverlay(markersLayer,"小区图层",'业务');
}
//当前热力图渲染
function addResultNow(arr,hotspot,flag){
    var heatIndexName = '业务用户数';
    var time = getCurrentTimeMin(15);//当前热力图渲染时间
    var url;
    var heatMap = [];
    heatIndexName === '信令用户数' && (url = 'http://' + baseUrl + ':8080/stream/totalnum/sites?time=' + time + '&hotspot=' + encodeURIComponent(hotspot));
    heatIndexName === '业务用户数' && (url = 'http://' + baseUrl + ':8080/stream/usernum/sites?time=' + time + '&hotspot=' + encodeURIComponent(hotspot));
    $.ajax({
        url: url,
        type: 'get',         //数据发送方式
        dataType: 'json',     //接受数据格式
        ...
        .done(function(data){
            ...
            heatMap = arr;
            heatMapRender(heatMap,flag);
        });
}
function heatMapRender (heatMap,flag) {
    //console.log(flag)
