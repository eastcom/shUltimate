...
function rerenderHeatMap(){
    ...
    if(!isInScreen(cacheHeatMapData)) return;//热力渲染区域不在屏幕范围内不渲染
...
function addRectangleTool(){
    ...
    map.on('draw:created', function (e) {
        ...
        //根据图层类别，在这儿定义查询事件
        if (type === 'rectangle' ){
            drawnItems && (map.removeLayer(drawnItems),drawnItems.clearLayers());
            ...
            var j = 1;
            var temStr = '';
            for(var i= 0,lenI= temSelectedLine.length; i<lenI; ++i){
                var lat = temSelectedLine[i][0],
                    lng = temSelectedLine[i][1];
                ...
                //temStr += 'id:' + (j++) + ',lat:' + lat + ',lng:' + lng + '</br>';
            }
            //$('#geoContent').html(temStr);
