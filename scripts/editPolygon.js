...
function initBaiduMap(){
    ...
    }).setView([31.146885,121.674619], 17);
    //自定义地图数据源
	//tileSize==512
...
function addRectangleTool(){
    ...
    map.on('draw:edited', function (e) {
        ...
        for(var i=0,lenI=layersArr.length; i<lenI; ++i){
            var latlngs = layersArr[i]._latlngs;
            var wgsPointArr = [];
            for(var j=0,lenJ=latlngs.length; j<lenJ; ++j){
                var lat = latlngs[j].lat,
                    lng = latlngs[j].lng;
                var wgsPoint = bd09towgs84(lng,lat);
                wgsPointArr.push({
                    lat: wgsPoint[1],
                    lng: wgsPoint[0]
                });
            }
            var name = layersArr[i]._popup._content;
            resultArr.push({
                name: name,
                latlngs: wgsPointArr
            });
        ...
        //console.log(resultArr);
        $('#geoContent').html(JSON.stringify(resultArr));
    });
}
//移动后的扇形分组
...
function beginAddXQ(arr,hotspot){
    ...
    hotspot === '交通枢纽' && (center = [31.195769,121.338157]);
    //console.log(center);
    map.panTo([31.144907,121.674224],{});
...
function callDivYHSMax(str){
    ...
    var winHeight = $(document).height(),
        winWidth = $(document).width();
    $('#userData1').css({position: 'absolute', width: '880px', height: '480px', left: winWidth/4, top: winHeight/7, 'background-color': '#252c34', border: 'none'});
...
function callDivYHSNormal(){
    ...
    $('#userData1').css({position: 'absolute', width: '600px', height: '300px', left: '80px', top: '40px', 'background-color': '#252c34', border: 'none'});
...
function callDivRMYYMax(str){
    ...
    var winHeight = $(document).height(),
        winWidth = $(document).width();
    $('#hotTerminal1').css({position: 'absolute', width: '880px', height: '480px', left: winWidth/4, top: winHeight/7, 'background-color': '#252c34', border: 'none'});
...
function callDivRMYYNormal(){
    ...
    $('#hotTerminal1').css({position: 'absolute', width: '600px', height: '300px', left: '80px', top: '680px', 'background-color': '#252c34', border: 'none'});
...
function getParasToOpenYHS(lacci,hotspot){
    var str = "pagesDSN/trendPlots/siteUserCountTime.html?"+lacci+"&"+hotspot;
    callDrawTrendPlot(str);
}
function getParasToOpenTotalYHS(lacci,hotspot){
    var str = "pagesDSN/trendPlots/siteSignalUserCountTime.html?"+lacci+"&"+hotspot;
    callDrawTrendPlot(str);
}
function getParasToOpenLL(lacci,hotspot){
    var str = "pagesDSN/trendPlots/siteTotalDataThroughPutTime.html?"+lacci+"&"+hotspot;
    callDrawTrendPlot(str);
}

//添加50个片区
function addAllDis(){
    var url = 'http://' + baseUrl + ':8080/services/ws/fast_query/area/re/re_areaHotRel?hotspot=迪士尼1&hot_type=1&isAllInfo=true';
    $.ajax({
        url: url,
        type: 'GET',
        dataType: 'json',
        ...
        .done(function(data){
            ...
            for(var i= 0,lenI=data.length; i<lenI; ++i){
                ...
                L.polygon(bdLatlngsArr).bindPopup(name).addTo(pianquLayer);
            }
